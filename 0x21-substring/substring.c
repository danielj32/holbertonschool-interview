...
/**
 * continus - chek if all words are continous
...
/**
 * finding - finding words in string
 * @s: string base
...
/**
 * find_substring - finding words in string
 * @s: string base
 * @words: array of words to searh in the string
 * @nb_words: number of words
 * @n: return size
...
int *find_substring(char const *s, char const **words, int nb_words, int *n)
{
	...
	*n = 0;
	...
	for (j = 0; j < s_len; j++)
	{
		aux_indx = finding(s, words, nb_words);
		if (!aux_indx)
			break;
		qsort(aux_indx, nb_words, sizeof(int), comp_twovalues);
		f_indx = continus(aux_indx, wordlen, nb_words);
